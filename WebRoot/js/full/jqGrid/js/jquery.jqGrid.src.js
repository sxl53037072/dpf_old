				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							...
							if($.isFunction(ts.p.onLoadErrorMsg) && typeof(data.succeed) !== "undefined" && !data.succeed){
									ts.p.onLoadErrorMsg.call(ts,data);
									endReq();
									return;
							}
						////lixj 新增onLoadErrorMsg 事件////
...
//			lixj 修改，去掉原有的a标签拦截，即点击a标签仍然可以选中行，但同时带来的问题就是他也会触发onselect事件	
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]=="both") {
				...
				$(grid.uDiv).insertBefore(grid.hDiv).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id).css({ height: 25+"px",padding:0});
				$(grid.ubDiv).insertAfter(grid.hDiv).addClass("ui-userdata ui-state-default").attr("id","tb_"+this.id).css({ height: 25+"px",padding:0});
				$(ts).jqGrid("addToToolBar","t_"+this.id,ts.p.mytoolbar);
				$(ts).jqGrid("addToToolBar","tb_"+this.id,ts.p.mytoolbar);
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				$(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id).css({ height: 25+"px",padding:0});
				$(ts).jqGrid("addToToolBar","t_"+this.id,ts.p.mytoolbar);
			}
	 ...
	 addToToolBar:function(id,toolbar){
		...
		if (toolbar) {
			...
			for(var i=0; i<toolbar.length; i++) {
				...
				var divM = $('<div class="btn-primary_wk_m"></div>');
				var divR = $('<div class="btn-primary_wk_r">&nbsp;</div>');
				var tool = $('<input type="button">');
