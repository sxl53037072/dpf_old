...
$.widget( "cg.combogrid", {
	...
	_create: function() {
	    ...
	    var $span = $("<span class='cg-combo-arrow'></span>");
	    ...
	    if (!self.options.searchButton){
		    $span.bind("click", function(event) {
				...
				if(!self.options.searchButton) {
					if(self.term == undefined) {
						...
						if(self.menucombo.element.children().length > 0) {
							if(self.menucombo.element.is(":visible")) {
								self.menucombo.element.hide();
							}
							else {
								self.menucombo.element.show();
								self.menucombo.activate($.Event({ type: "mouseenter" }), self.menucombo.itemsEle.children(".cg-menu-item:first"));
							}
	    	...
	    	.addClass("combo-arrow-input")
	    	.width(parseInt(this.element.css('width')) - 16);
	    }
		this.element
			...
			.bind( "keyup.combogrid", function( event ) {
...
//				hasFocus++;
				...
				self.menucombo.element.zIndex(self.element.zIndex() + 1);
			})
/*			.bind( "click.combogrid", function(event) {
				if ( self.options.disabled ) {
					return;
				}
				if(!self.options.searchButton) {
					if(hasFocus > 1) {
						if(hasFocus == 2) {
							if(self.element.val().trim() == '' && self.term == undefined) {
								self.term = self.element.val();
								self.search(null, event);
							}
						}
			...
			.bind( "blur.combogrid", function( event ) {
				...
				self.clear();
				self._trigger("onBlur", event);
			})
			.bind("init.combogrid", function(event) {
				if ( self.options.disabled ) {
					return;
				}
				isClickSpan = true;
			...
			var isHaveSetParams = $.isFunction(self.options.setParams);
			if(isHaveSetParams) {
				var returnParamValue = self.options.setParams(1);
				if(!$.isPlainObject(returnParamValue)) {
					isHaveSetParams = false;
				}
			}
...
/*					data: {
					...
					data:isHaveSetParams ? $.extend({}, {sidx: self.options.sidx,
														 page : self.page,
														 sord : self.options.sord,
														 rows: self.options.rows}
													  , self.options.setParams(request.term)) 
										 : $.extend({}, {sidx: self.options.sidx,
														 page : self.page,
														 sord : self.options.sord,
														 rows: self.options.rows}
													  , {searchTerm : request.term}),
					...
					success: function( data ) {
						if(data.totalCount==0) {
							...
							if ( !self.pending ) {
								...
								self.menucombo.element.empty();
								self.close();
							}
						} else if(data.totalCount==1) {
							response(data.totalCount, data.totalPage, $.map( data.rows, function( item ) {
								return item;
							}));
							self.menucombo.activate($.Event({ type: "mouseenter" }), self.menucombo.itemsEle.children(".cg-menu-item:first"));
							if(self.options.autoChoose || self.options.initLoad){
								setTimeout(function() {
						...
						if(self.options.initLoad) {
							self.menucombo.element.hide();
							setTimeout(function() {self.element.blur();}, self.options.delayChoose);
							self.options.initLoad = false;
						}
					}
				});
			};
		...
		this.menucombo = $( "<div style='border: 1px solid #CCCCCC;'></div>" )
			...
			.menucombo({
				...
				selected: function( event, ui ) {
					...
					self.term = self.element.val();
					self.selectedTerm = self.element.val();
					var rows = [];
					rows[0] = item;
					self.pending++;
				    self.page = 1;
				    self._response(1, 1, rows);				    
	...
	/**
	 * 初始化自动加载数据，如果数据只有一条则自动填充
	 */
	initLoad : function() {
		this.options.initLoad = true;
		this.element.trigger("init.combogrid");
	},
	
	/**
	 * 没有通过下拉框选择，则失去焦点时自动清空输入框的值
	 */
	clear: function() {
		if(this.selectedTerm != this.element.val()) {
...
//			$('.' +self.element.attr('id') +'.cg-keynav-first').unbind('click.combogrid');
	...
	_suggest: function(records, total, items ) {
		var self = this;
		var ul = this.menucombo.element.empty()
			.zIndex( this.element.zIndex() + 1 );
...
//		ul.eq(0).empty();
		$itemsDIv.prev().empty();
		$itemsDIv.next().empty();*/
		var $itemsDiv = $("<div class='cg-items-all'>");
		if(self.options.height) {
			$itemsDiv.css("height", self.options.height);
		}
		if(self.options.maxHeight) {
			$itemsDiv.css("max-height", self.options.maxHeight);
		}
		$('.' +self.element.attr('id') +'.cg-keynav-next').unbind('click.combogrid');
		$('.' +self.element.attr('id') +'.cg-keynav-prev').unbind('click.combogrid');
		$('.' +self.element.attr('id') +'.cg-keynav-last').unbind('click.combogrid');
		...
		this._renderMenu( $itemsDiv, items, this.options.colModel );
		ul.append($itemsDiv);
...
//		ul2.appendTo(ul);
	...
	_renderPager: function( ul, records, total ) {
		...
		div = $("<div class='cg-comboButton ui-state-default' style='padding-right:0px;'>");
...
// Select page
						+"<td dir='ltr' style='padding-left:5px;'>"
							+"<select class='" +self.element.attr('id') + " recordXP rowsOfPage'>"
							+"</select>"
						+"</td>"
					+"</tr>"
				+"</tbody>"
	...
	_renderPagerPage: function (label, page, total){
		...
		if(this.options.i18n){
			...
			return '第 <input type="text" size="1" class="' +self.element.attr('id')+ ' currentPage" value="' + page + '"></input> 页 共 ' + total + ' 页';
		}
	},
...
//			return "View " + initRecord + " - " + lastRecord + " of " + records;
			return "显示 " + initRecord + " - " + lastRecord + " 条 共 " + records + " 条";
		}
	},
...
$.widget("cg.menucombo", {
	...
	refresh: function() {
		this.itemsEle = this.element.children().eq(1);
		var self = this;
        
		// don't refresh list items that are already adapted
		var items = this.itemsEle.children("div:not(.cg-menu-item):not(#cg-divHeader):not(.cg-comboButton):has(div)")
	...
	move: function(direction, edge, event) {
		if (!this.active) {
			this.activate(event, this.itemsEle.children(edge));
			return;
		}
	...
	nextPage: function(event) {
		if (this.hasScroll()) {
			...
			if (!this.active || this.last()) {
				this.activate(event, this.itemsEle.children(".cg-menu-item:first"));
				return;
			}
			var base = this.active.offset().top,
				height = this.element.height(),
				result = this.itemsEle.children(".cg-menu-item").filter(function() {
			...
			if (!result.length) {
				result = this.itemsEle.children(".cg-menu-item:last");
			}
			this.activate(event, result);
		} else {
			this.activate(event, this.itemsEle.children(".cg-menu-item")
	...
	previousPage: function(event) {
		if (this.hasScroll()) {
			...
			if (!this.active || this.first()) {
				this.activate(event, this.itemsEle.children(".cg-menu-item:last"));
				return;
			}

			var base = this.active.offset().top,
				height = this.itemsEle.height();
				result = this.itemsEle.children(".cg-menu-item").filter(function() {
					var close = $(this).offset().top - base + height - $(this).height();
					// TODO improve approximation
					return close < 10 && close > -10;
			...
			// TODO try to catch this earlier when scrollTop indicates the last page anyway
			if (!result.length) {
				result = this.itemsEle.children(".cg-menu-item:first");
			}
			this.activate(event, result);
		} else {
			this.activate(event, this.itemsEle.children(".cg-menu-item")
